...
type OrderInfo struct {
	...
	UpdatedAt                     time.Time              `gorm:"comment:更新时间;not null;column:updated_at;default:CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3);" json:"-"`
	DeletedAt                     gorm.DeletedAt         `gorm:"column:deleted_at;index" json:"-"`
	UserID                        string                 `gorm:"column:user_id;type:varchar(100);default:'';not null;comment:下单用户ID;index:order_sn_status_user" json:"userID"`
	...
	SignerAddress                 string                 `gorm:"column:signer_address;type:varchar(300);comment:收件人的地址"`
	...
	CloseReason                   string                 `gorm:"column:close_reason;type:varchar(500);comment:结束订单留言信息"`
